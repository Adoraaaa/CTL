function [I,Icell,Io,Iocell] = CreateDataRe(param, I, CONTRAST_NORMALIZE, ZERO_MEAN, COLOR_TYPE, SQUARE_IMAGES, image_frames)
...
if param.displayc
    ...
    switch(COLOR_TYPE)
        case 'rgb'
            fprintf('Making RGB Image %10d\r',actual_files);
        case 'ycbcr'
            fprintf('Making YUV Image %10d\r',actual_files);
        case 'hsv'
            fprintf('Making HSV Image %10d\r',actual_files);
        case 'gray'            
            fprintf('Making Gray Image %10d\r',actual_files);
    end
end
for i=1:length(I)
    switch(COLOR_TYPE)
...
switch(CONTRAST_NORMALIZE)
    ...
    case 'local_cn'
    fprintf('Contrast Normalizing Image with Local CN: %10d\r',actual_files);
end
end
switch(CONTRAST_NORMALIZE)
...
Icell = reshape(I,[1 numims]);

I = single(cell2mat(Icell));
I = reshape(I,[xdim ydim numims colors]);
I = permute(I,[1 2 4 3]);
I = double(I);

Iocell=Io;
Io = single(cell2mat(Io));
Io = reshape(Io,[xdim ydim numims colors]);
Io = permute(Io,[1 2 4 3]);
Io = double(Io);
