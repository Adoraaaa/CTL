function [f ] = HashingHist(CTL,ImgIdx,OutImg)
...
NumFilters=CTL.NumFilters(CTL.layers);

NumImg = max(ImgIdx);
f = cell(NumImg,1);
map_weights = 2.^((NumFilters(end)-1):-1:0); % weights for binary to decimal conversion

for Idx = 1:NumImg
  
    Idx_span = find(ImgIdx == Idx);
    NumOs = length(Idx_span)/NumFilters(end); % the number of "O"s
    Bhist = cell(NumOs,1);
    
    for i = 1:NumOs 
        
        T = 0;
        ImgSize = size(OutImg{Idx_span(NumFilters(end)*(i-1) + 1)});
        for j = 1:NumFilters(end)
            T = T + map_weights(j)*Heaviside(OutImg{Idx_span(NumFilters(end)*(i-1)+j)}); 
            % weighted combination; hashing codes to decimal number conversion
            
            OutImg{Idx_span(NumFilters(end)*(i-1)+j)} = [];
        end
        
        
        if isempty(CTL.HistBlockSize)
            NumBlk = ceil((CTL.ImgBlkRatio - 1)./CTL.BlkOverLapRatio) + 1;
            HistBlockSize = ceil(size(T)./CTL.ImgBlkRatio);
            ...
            OverLapinPixel),(0:2^NumFilters(end)-1)')); 
        else 
            
            stride = round((1-CTL.BlkOverLapRatio)*CTL.HistBlockSize); 
            blkwise_fea = sparse(histc(im2col_general(T,CTL.HistBlockSize,...
              stride),(0:2^NumFilters(end)-1)')); 
            % calculate histogram for each local block in "T"
            
           if ~isempty(CTL.Pyramid)
                x_start = ceil(CTL.HistBlockSize(2)/2);
                y_start = ceil(CTL.HistBlockSize(1)/2);
                x_end = floor(ImgSize(2) - CTL.HistBlockSize(2)/2);
                y_end = floor(ImgSize(1) - CTL.HistBlockSize(1)/2);
                
                sam_coordinate = [...
                    kron(x_start:stride:x_end,ones(1,length(y_start:stride: y_end))); 
                    kron(ones(1,length(x_start:stride:x_end)),y_start:stride: y_end)];               
                
                blkwise_fea = spp(blkwise_fea, sam_coordinate, ImgSize, CTL.Pyramid)';
                
           else
                blkwise_fea = bsxfun(@times, blkwise_fea, ...
                    2^NumFilters(end)./sum(blkwise_fea)); 
           end
